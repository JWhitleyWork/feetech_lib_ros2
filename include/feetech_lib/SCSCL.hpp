...
public:
  SCSCL();
  explicit SCSCL(uint8_t End);
  SCSCL(uint8_t End, uint8_t Level);
  virtual int WritePos(
                               // 普通写单个舵机位置指令
    uint8_t ID, uint16_t Position, uint16_t Time, uint16_t Speed = 0);
  virtual int RegWritePos(
                              // 异步写单个舵机位置指令(RegWriteAction生效)
    uint8_t ID, uint16_t Position, uint16_t Time, uint16_t Speed = 0);
  virtual void SyncWritePos(
                              // 同步写多个舵机位置指令
    uint8_t ID[], uint8_t IDN, uint16_t Position[], uint16_t Time[], uint16_t Speed[]);
